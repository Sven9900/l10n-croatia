...
from . import joppd_help as jh, joppd_schema_specific as jss
...
class Joppd(models.Model):
    ...
    name = fields.Char(
        ...
        index=True,
        default=_get_name_draft,
    )
    oznaka = fields.Char(
        string="I. Oznaka izvješća",
        size=5,
        required=True,
        readonly=True,
        states={"draft": [("readonly", False)]},
    ...
    parent_id = fields.Many2one(comodel_name="l10n.hr.joppd", string="Izvorno izvješće")
    ...
    podnositelj_oznaka = fields.Selection(
        # TODO mozda m2o na codebook?
        selection=_get_oznaka_selection,
        default="1",
    ...
    xml_schema = fields.Selection(
        selection=[("1.0", "v1.0 - od 1.1.2015"), ("1.1", "v1.1 - od 28.2.2015")],
    ...
    def button_done_editing(self):
        ...
        self.name = "_".join(
            (
                ...
                self.vrsta,
                self.oznaka,
                self.company_id.get_l10n_hr_time_formatted()["datum_vrijeme"].replace(
                    "T", "_"
                ),
            )
        )
    ...
    def button_generate_xml(self):
        ...
        joppd = JOPPD.ObrazacJOPPD(
            metadata, StranaA, StranaB, verzijaSheme=self.xml_schema
        )
...
class Joppd_A(models.Model):
    ...
    joppd_id = fields.Many2one(
        comodel_name="l10n.hr.joppd", string="JOPPD obrazac", ondelete="cascade"
    )
    code = fields.Char(string="Pozicija", required=True)
    position = fields.Char(string="Opis pozicije", required=True)
    value = fields.Float(string="Iznos", copy=False)  # TODO: define decimal precision
...
class Joppd_B(models.Model):
    ...
    original_line_id = fields.Many2one(
        comodel_name="l10n.hr.joppd.b", string="Izvorni redak"
    )
    ...
    b1 = fields.Integer(string="1. Redni broj", readonly=True)
    b2_id = fields.Many2one(
        string="2. Šifra općine/grada prebivališta/boravišta",
        comodel_name="l10n.hr.joppd.codebook",
        domain=[("usage", "in", ["city", "country"])],
        required=True,
        help="Šifrarnik gradova ili država, npr: Zagreb: 01333",
    )
    b3_id = fields.Many2one(
    ...
    b4 = fields.Char(string="4. OIB stjecatelja/osiguranika", size=11, required=True)
    b5 = fields.Char(
        string="5. Ime i prezime stjecatelja/osiguranika", size=128, required=True
    )
    # b61 = fields.Char(
    #     string='6.1. Oznaka stjecatelja/osiguranika',
    #     size=6, required=True, ) # default='0001'
    b61_id = fields.Many2one(
        string="6.1. Oznaka stjecatelja/osiguranika",
        comodel_name="l10n.hr.joppd.codebook",
        domain=[("usage", "=", "P-2")],
        required=True,
        help="šifre: Prilog 2, npr: 0001",
    )
    # b62 = fields.Char(
    #     string='6.2. Oznaka primitka/obveze doprinosa',
    #     size=6, required=True, ) # default='0001'
    b62_id = fields.Many2one(
        string="6.2. Oznaka primitka/obveze doprinosa",
        comodel_name="l10n.hr.joppd.codebook",
        domain=[("usage", "=", "P-3")],
        required=True,
        help="šifre: Prilog 3, npr: 0001",
    )
    ...
    b10 = fields.Integer(string="10. Broj sati rada", help=jh.b10_help)
    b100 = fields.Integer(string="10.0. Broj neodrađenih sati", help=jh.b10_help)

    b101 = fields.Date(
        string="10.1. Razdoblje OD", required=True, help=jh.b_101_102_help
    )
    b102 = fields.Date(
        string="10.2. Razdoblje DO", required=True, help=jh.b_101_102_help
    )
    b11 = fields.Monetary(
        string="11. Iznos primitka (oporezivi)", currency_field="currency_id"
    )
    b12 = fields.Monetary(
    ...
    b122 = fields.Monetary(
        string="12.2. Dopr. za mir. osig. II Stup", currency_field="currency_id"
    )
    b123 = fields.Monetary(
        string="12.3. Zdravstveno osig.", currency_field="currency_id"
    )
    b124 = fields.Monetary(
        string="12.4. Doprinos za zašt.zdravlja na radu", currency_field="currency_id"
    )
    b125 = fields.Monetary(
        string="12.5. Doprinosi za zapošljavanje", currency_field="currency_id"
    )
    ...
    b128 = fields.Monetary(
        string="12.8. Poseban dop. - zdrav.zašt.ino.", currency_field="currency_id"
    )
    b129 = fields.Monetary(
        string="12.9. Poseban dop. za zap.osoba s inv.", currency_field="currency_id"
    )
    b131 = fields.Monetary(string="13.1. Izdatak", currency_field="currency_id")
    b132 = fields.Monetary(
        string="13.2. Izdatak -upla.dop. mir.osig", currency_field="currency_id"
    )
    b133 = fields.Monetary(string="13.3. Dohodak", currency_field="currency_id")
    b134 = fields.Monetary(string="13.4. Osobni odbitak", currency_field="currency_id")
    b135 = fields.Monetary(
        string="13.5. Porezna osnovica", currency_field="currency_id"
    )
    b141 = fields.Monetary(
        string="14.1. Izn.obr. poreza na dohodak", currency_field="currency_id"
    )
    b142 = fields.Monetary(
        string="14.2. Izn.obr. prireza por.na doh.", currency_field="currency_id"
    )
    # b151 = fields.Char(
    #     string='15.1. Oznaka neopor. primitka',
    #     size=5, required=True, )  # default='0'
    b151_id = fields.Many2one(
        string="15.1. Oznaka neopor. primitka",
        comodel_name="l10n.hr.joppd.codebook",
        domain=[("usage", "=", "P-4")],
        required=True,
        help="šifre: Prilog 4, npr: 0",
    )
    b152 = fields.Monetary(
        string="15.2. Iznos neopor. primitka", currency_field="currency_id"
    )
    # b161 = fields.Char(
    #     string='16.1. Oznaka načina isplate',
    #     size=5, required=True, )  # default='0'
    b161_id = fields.Many2one(
        string="16.1. Oznaka načina isplate",
        comodel_name="l10n.hr.joppd.codebook",
        domain=[("usage", "=", "P-5")],
        required=True,
        help="šifre: Prilog 5, npr: 0",
    )
    b162 = fields.Monetary(
        string="16.2. Iznos za isplatu", currency_field="currency_id"
    )
    b17 = fields.Monetary(
        string="17. Obračun. prim. od nesam. rada (plaća)", currency_field="currency_id"
    )
    ...
    def onchange_employee_id(self):
        if not self.employee_id:
            ...
            self.b5 = self.employee_id.name
            self.b4 = self.employee_id.l10n_hr_oib
            self.b2_id = self.employee_id.l10n_hr_joppd_b2_id
            self.b3_id = self.employee_id.l10n_hr_joppd_b3_id
