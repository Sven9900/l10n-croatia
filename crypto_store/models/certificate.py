...
class CryptoCertificate(models.Model):
    ...
    usage = fields.Char(compute="_compute_usage")
    ...
    def _check_valid(self):
        ...
        if self.status == 'valid_request' and self.state == 'draft':
            state = 'waiting'
        elif self.status in ('valid', 'certificate_converted') and self.state in ['draft', 'waiting']:
            state = 'confirmed'
        else:
    ...
    def button_convert_p12(self):
        ...
        if self.cert_file:
            ...
            try:
                ...
                raise UserError(_('Certificate acces error, check password or file type!'))
